...
**Cause:** Power BI autotyped the column as a number; 14-digit integers exceed clean display precision and identifiers should never be numeric anyway (they are never summed, and leading zeros matter in other datasets).

**Fix:** retyped `API_WELLNO` and `FileNo` as **Text** immediately after import, before any other transformation.
